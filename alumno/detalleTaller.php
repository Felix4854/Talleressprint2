        ...
        <div id="wrapper">
            <div id="layout-static">
                <div class="static-sidebar-wrapper sidebar-midnightblue">
                    ...
                    $id_alum = $_SESSION['alum'];
 $sqlAlumnos = "SELECT `inscrito`.`id` AS `id`, `alumno`.`id` AS `ida`, `alumno`.`dni` AS `dni`, `alumno`.`nombres` AS `nombres`, `alumno`.`correo` AS `correo` FROM `alumno`, `inscrito` WHERE `alumno`.`estado` = 1 AND `inscrito`.`alumno_id` = `alumno`.`id` AND `inscrito`.`taller_id`='$id' AND `inscrito`.`estado`=1  AND `alumno`.`id`='$id_alum' ;";
    $query_alum=mysqli_query($con, $sqlAlumnos); 
        ...
        <div class="options">
            <div class="btn-toolbar">              
                <a href="inscribir.php?id=<?php echo $id ?>" class="btn btn-success"><i class="fa fa-plus-circle"></i> INSCRIBIR ALUMNO</a>
...
?>  
                        <tr>                            
                            ...
                            <td>
                                <a href="asistenciaTaller.php?id=<?php echo $rowc['id']; ?>" class="btn btn-success"><i class="fa fa-pencil"></i> Asistencia</a>
                            </td>
                        </tr>
<?php endwhile; ?>                        
                    </tbody>
