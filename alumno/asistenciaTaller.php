<?php 
    ...
    $sql_inscrito = "SELECT `id` AS `id_ins` FROM `inscrito` WHERE `alumno_id`='$id' AND `taller_id` = '$id_t'";
    $query_ins = mysqli_query($con, $sql_inscrito);
    $row_ins = mysqli_fetch_array($query_ins);
    $id_in = $row_ins['id_ins'];
    ...
    $sqlAlumno = "SELECT `nombres` FROM `alumno` WHERE `id`='$id';";
    $query_alum=mysqli_query($con, $sqlAlumno);
    $rowAl = mysqli_fetch_array($query_alum);
    //-------------- lista de asistencias -----------------
    $sql_asistencia = "SELECT `id`,DATE_FORMAT(`fecha`,'%d/%m/%y') AS `fecha`, DATE_FORMAT(`entrada`,'%H:%i') AS `entrada`, DATE_FORMAT(`salida`,'%H:%i') AS `salida` FROM `asistencia` WHERE `inscrito_id`='$id_in' AND `alumno_id`='$id';";
...
<!-- fin de cabecera -->
        ...
        <div class="options">
            <div class="btn-toolbar">              
                ...
                <a href="detalleAlumno.php?id=<?php echo $id?>" class="btn btn-info"><i class="fa  fa-reply"></i> REGRESAR</a>
            </div>
        </div>
    </div>
